# Unit のセキュリティ（デフォルトから変更したほうが良い設定）

## 概要

Ansible を利用してPersonium ユニットを作成した場合、デフォルトでユニットマスタートークンといった強い権限を持つトークンが有効になっています。本説明ではそういったデフォルトから変更したほうが良い設定についてご説明します。

デフォルトから変更したほうが良い設定には以下のようなものがあります。
1. 有効にされたユニットマスタートークン
1. ユニット管理アカウント
...
1. '/personium/personium-core/conf/18888/personium-unit-config.properties' の修正
    ...
    * io.personium.core.masterToken パラメータをコメントアウトする事でユニットマスタートークンは無効化されます。
